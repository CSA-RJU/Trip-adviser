...
def submit(*args): #Adds 34 percent to progressbar if hassent already, adds up price and displays it.
    if couLB.curselection()==():
        print ("Please choose a country!")
        clear()
    elif spinvar.get()=="V-Month-V":
        print ("Please choose a Month!")
        clear()
    elif combovar.get()=="V-Type of Travel-V":
        print ("Please choose a Type of Travel!")
        clear()
    else:
        ledcheck=open("trip_ledge.txt", "a")
        ledcheck.write("Country: " +str(couLB.get(ACTIVE))+ ", Month: " +str(spinvar.get())+ ", Mode of Travel: " +str(combovar.get())+ ", Notes: " +str(notT.get(1.0, END))) #Shows all applied data.
        ledcheck.write("\n====================================================================================\n")  #For organising.
        ledcheck.close()
...
notL.grid(column=20, row=19, columnspan=30, sticky="NSEW")
...
SB=Spinbox(root, state="readonly", values=SB_list, textvariable=spinvar)
...
root.title("Trip Logger") #Makes the title what is in the "s.
